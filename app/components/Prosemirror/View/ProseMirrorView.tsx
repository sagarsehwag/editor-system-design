  ...
  return (
    <div className="view-tab">
      ...
      {activeSubTab === 'overview' && (
        <>
          <div className="prosemirror-section view-intro">
            <h3>What is the View?</h3>
            <p>
              The <strong>EditorView</strong> is the UI layer. It reads <code>state.doc</code> and
              renders it to the DOM, handles user input (keyboard, mouse, clipboard), and dispatches
              transactions when the user edits.
            </p>
            <p>
              You create it with <code>new EditorView(element, {`{ state, dispatch }`})</code>. The view
              subscribes to state updates and re-renders when state changes.
            </p>
          </div>
          <div className="prosemirror-section">
            <h4>State → View → Dispatch</h4>
        <p>
          The view reads state to render. When the user types or selects, the view creates a
          transaction and calls <code>dispatch(tr)</code>. The owner (e.g. React) applies it and
      ...
      <div className="prosemirror-section">
        ...
        <ul className="view-responsibilities-list">
          <li>
            <strong>Rendering</strong> — Converts <code>state.doc</code> to DOM nodes. Uses a
            node-view system for custom rendering.
          </li>
          <li>
            <strong>Input handling</strong> — Keyboard, mouse, clipboard. Maps browser events to
            transactions.
          </li>
          <li>
            <strong>Selection</strong> — Syncs <code>state.selection</code> with the DOM selection.
          </li>
          <li>
            <strong>Props</strong> — <code>editable</code>, <code>attributes</code>, event handlers.
            Passed when creating the view.
          </li>
        </ul>
      </div>
  ...
  dispatchTransaction(tr) {
    ...
    view.updateState(newState);
        ...
        <p className="section-note">
          ...
          create the view and wire <code>dispatchTransaction</code> to your state setter.
        </p>
      </div>
        </>
      )}
      ...
      {activeSubTab === 'decorations' && (
        <div className="prosemirror-section view-decorations">
          <h3>Decorations</h3>
          <p>
            <strong>Decorations</strong> are visual overlays the view renders on top of the document
            without changing its content. They&apos;re provided via <code>props.decorations</code> —
            typically from plugins — and used for search highlights, syntax highlighting, placeholders,
            and inline widgets.
          </p>
    ...
    decorations(state) {
      ...
      return DecorationSet.create(state.doc, [
        Decoration.inline(from, to, { class: "highlight" })
          ...
          <p className="section-note">
            The view merges all <code>props.decorations</code> from plugins and renders them when
            drawing the document. <code>DecorationSet.map(tr.mapping)</code> updates positions when
            the doc changes.
          </p>
        </div>
      )}
