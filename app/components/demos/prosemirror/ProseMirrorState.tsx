...
export default function ProseMirrorState() {
  return (
    <div className="state-tab">
      <div className="prosemirror-section state-intro">
        <h3>What is EditorState?</h3>
        <p>
          <strong>EditorState</strong> is the single source of truth for the document, selection,
          stored marks, schema, and plugin state. It&apos;s immutable — you never mutate it; you
        ...
        <div className="state-single-source">
          <div className="state-ss-hub">
            ...
            <span className="state-ss-hub-label">All read from &amp; write to</span>
          </div>
          ...
          <div className="state-ss-items">
            <div className="state-ss-item">
              ...
              <span className="state-ss-desc">Writes → dispatches transactions</span>
            </div>
            <div className="state-ss-item">
              <span className="state-ss-label">Commands</span>
              <span className="state-ss-desc">Reads state → decides</span>
              <span className="state-ss-desc">Writes → creates transactions</span>
            </div>
            <div className="state-ss-item">
              <span className="state-ss-label">Plugins</span>
              <span className="state-ss-desc">Reads state → pluginState()</span>
      ...
      <div className="prosemirror-section">
        ...
        <p>
          Updates are never in-place. You create a transaction (steps) and apply it. That returns a
          new EditorState — the old one is unchanged. This enables undo, redo, and collaborative
          editing.
        </p>
        <div className="state-flow-visual">
          ...
          <div className="state-flow-arrow-wrap">
            ...
            <div className="state-flow-arrow">state.tr.insertText(&quot;x&quot;)</div>
          </div>
...
// state unchanged; newState has the new doc`}</code>
        ...
        <p className="state-callout">
          ...
          transaction you can chain steps on (<code>tr.insertText()</code>, <code>tr.delete()</code>, etc.).
        </p>
      </div>
      ...
      <div className="prosemirror-section">
        ...
        <p>
          To bootstrap an editor, use <code>EditorState.create()</code>. You need a schema and
  ...
  plugins: [history(), keymap(...)]
        ...
        <p className="section-note">
          ...
          history, keybindings, and more — see Plugins tab.
        </p>
      </div>
      ...
      <div className="prosemirror-section">
        ...
        <p>
          Every EditorState has these core fields. <code>doc</code> and <code>selection</code> are
          the ones you use most; plugins extend with their own state.
        </p>
        <div className="state-structure-merged">
          <div className="state-object-box">
            ...
            <div className="state-object-body">
              ...
              <div className="state-object-row">
                ...
                <span className="state-field-desc">Caret/selection (anchor, head)</span>
              </div>
              <div className="state-object-row">
                <span className="key">storedMarks</span>
                <span className="arrow">→</span>
                <span className="value">Mark[] | null</span>
                <span className="state-field-desc">Marks for next typed char</span>
              </div>
              <div className="state-object-row">
                <span className="key">schema</span>
                <span className="arrow">→</span>
                <span className="value">Schema</span>
                <span className="state-field-desc">Schema this state conforms to</span>
              </div>
              <div className="state-object-row">
                <span className="key">plugins</span>
                <span className="arrow">→</span>
                <span className="value">PluginState[]</span>
                <span className="state-field-desc">Plugin instances + state</span>
              </div>
            </div>
          </div>
          ...
          <p>
            <code>doc</code> holds the content; <code>selection</code> (from, to) selects a range.
            Together they describe what the user sees.
          </p>
          <div className="state-doc-selection-visual">
            <p className="state-ds-label"><code>from=1, to=6</code></p>
            <div className="state-doc-visual">
              {[
                [0, '<p>'], [1, 'H'], [2, 'e'], [3, 'l'], [4, 'l'], [5, 'o'], [6, ' '], [7, 'w'], [8, 'o'], [9, 'r'], [10, 'l'], [11, 'd'], [12, '</p>'],
              ].map(([pos, token]) => {
                const inSel = Number(pos) >= 1 && Number(pos) < 6;
                return (
                  <span key={pos} className={`state-doc-gap ${inSel ? 'in-selection' : ''}`}>
            ...
            <p className="state-ds-legend">Selects &quot;Hello&quot;. See Positions tab for details.</p>
          </div>
        </div>
      </div>
      ...
      <div className="prosemirror-section">
        <h4>Plugin State</h4>
        <p>
          Each plugin can store its own state. Access it via <code>state.pluginState(plugin)</code> or{' '}
          <code>plugin.getState(state)</code>. When a transaction is applied, plugins reconcile their
...
// or: historyPlugin.getState(state)`}</code>
      ...
      <div className="prosemirror-section">
        <h4>storedMarks</h4>
        <p>
          When the cursor is inside formatted text (e.g. bold), <code>storedMarks</code> holds those
          marks. The next character you type gets them applied automatically. When the cursor is in
          plain text, <code>storedMarks</code> is <code>null</code>.
        </p>
        <div className="state-storedmarks-visual">
          <div className="state-sm-panel before">
            <h5>Before typing</h5>
            <p>Cursor inside bold: Hello <strong>w|</strong>orld</p>
            <div className="state-sm-code"><code>storedMarks: [strong]</code></div>
            <p className="state-sm-hint">Next typed char will be bold.</p>
          </div>
          <div className="state-sm-panel after">
            <h5>After typing &quot;x&quot;</h5>
            <p>Result: Hello <strong>wx|</strong>orld</p>
            <div className="state-sm-code"><code>storedMarks: [strong]</code></div>
            <p className="state-sm-hint">The &quot;x&quot; inherited the bold mark.</p>
          </div>
        </div>
      </div>
    </div>
